...
public class Tab {
  ...
  public void SetContextTrans(Node p) { // set transition code in the graph rooted at p
		...
		if (ddt[4]) {
			...
			foreach (Node p in nodes)
				if (p.typ == Node.any || p.typ == Node.sync) {
					trace.Write("Line: {0, 4} Node: {1,4} {2,4}: ", p.line, p.n, nTyp[p.typ]);
					PrintSet(p.set, 11);
				}
		}
	}
	...
	int CheckOverlap(BitArray s1, BitArray s2, int cond) {
		int overlaped = 0;
		foreach (Symbol sym in terminals) {
				if (s1[sym.n] && s2[sym.n]) { LL1Error(cond, sym); ++overlaped; }
		}
		return overlaped;
    	}

	/* print the path for first set that contains token tok for the graph rooted at p */
	void PrintFirstPath(Node p, int tok, string indent = "\t", int depth = 0)
	{
		...
		while (p != null)
		{
			...
			case Node.nt:
			{
				if (p.sym.firstReady)
				{
					if (p.sym.first[tok])
					{
						if (indent.Length == 1) Console.WriteLine("{0}=> {1}:{2}:{3}:", indent, p.sym.name, p.line, p.col);
						Console.WriteLine("{0}-> {1}:{2}:{3}:", indent, p.sym.name, p.sym.line, p.sym.col);
			...
			case Node.wt:
			{
				if (p.sym.n == tok) Console.WriteLine("{0}= {1}:{2}:{3}:", indent, p.sym.name, p.line, p.col);
				break;
			}
			case Node.any:
			{
			...
			if (p.typ == Node.alt) {
				...
				while (q != null) { // for all alternatives
					...
					if (overlaped > 0)
					{
						...
						foreach (Symbol sym in terminals)
						{
						    if (s1[sym.n] && s2[sym.n]) { overlapToken = sym.n; break; }
						}
